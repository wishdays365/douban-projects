...
# 全局配置：解决中文乱码
plt.rcParams["font.sans-serif"] = ["SimHei", "Arial Unicode MS", "PingFang SC", "Heiti TC", "WenQuanYi Micro Hei", "DejaVu Sans"]
...
class DoubanTop250Visualizer:
    ...
    def plot_genre_bar(self):
        ...
        plt.close()
        logger.info(f"类型分布图已保存：{img_path}")
        return img_path
    ...
    def plot_country_bar(self):
        ...
        plt.savefig(img_path, dpi=100, bbox_inches="tight")
        plt.close()
        logger.info(f"国家分布图已保存：{img_path}")
        return img_path
    ...
    def plot_actor_bar(self):
        ...
        plt.savefig(img_path, dpi=100, bbox_inches="tight")
        plt.close()
        logger.info(f"演员出镜图已保存：{img_path}")
        return img_path
...
def generate_visual_menu(visualizer):
    while True:
        logger.info("===== 可视化图表生成菜单 =====")
        logger.info("1. 电影类型分布柱状图")
        logger.info("2. 国家/地区分布柱状图")
        logger.info("3. 演员出镜次数柱状图")
        logger.info("4. 返回主菜单")
