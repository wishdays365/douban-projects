...
class DoubanTop250Table:
    ...
    def _load_and_process_data(self):
        ...
        try:
            ...
            if not os.path.exists(self.csv_path):
                logger.warning(f"未找到数据文件：{self.csv_path}，请先爬取数据！")
                return
            ...
            logger.info(f"成功加载 {len(self.processed_data)} 条电影数据")
    ...
    def show_terminal_table(self, limit=None):
        ...
        if not self.is_data_loaded or not self.processed_data:
            logger.info("无有效数据可展示，请先确认数据已爬取！")
            return
    ...
    def export_excel(self, output_path="douban_top250_table.xlsx", sheet_name="电影数据"):
        ...
        """
        if not self.is_data_loaded or not self.processed_data:
            logger.info("无有效数据可导出，请先确认数据已爬取！")
            return
        ...
        try:
            ...
            logger.info(f"Excel表格已导出至：{os.path.abspath(output_path)}")
        except Exception as e:
            logger.exception(f"Excel导出失败：{str(e)}")
            logger.info("请安装依赖：pip install openpyxl pandas")
...
def generate_table_menu(table_tool):
    ...
    while True:
        logger.info("----- 制表功能子菜单 -----")
        logger.info("1. 终端展示前10条数据")
        logger.info("2. 终端展示全部数据")
        logger.info("3. 导出Excel表格")
        logger.info("4. 返回主菜单")
