...
class MovieAnalyzer:
    def __init__(self, json_path):
        ...
        self.movies = self._load_json_data()  # 直接加载JSON（适配已有JSON文件）
    ...
    def _load_json_data(self):
        ...
        for movie in movies:
            ...
            # 数组字段默认值（避免None/非列表类型）
            movie["director"] = movie.get("director", []) if isinstance(movie.get("director"), list) else []
            movie["actors"] = movie.get("actors", []) if isinstance(movie.get("actors"), list) else []
            movie["genre"] = movie.get("genre", []) if isinstance(movie.get("genre"), list) else []
            movie["country"] = movie.get("country", []) if isinstance(movie.get("country"), list) else []
            movie["language"] = movie.get("language", []) if isinstance(movie.get("language"), list) else []
        return movies
    ...
    def country_analysis(self):
        ...
        # 新增：打印统计结果，方便排查美国数据是否正确
        country_result = dict(country_counter.most_common(10))
        print(f"📊 国家统计结果：{country_result}")
    ...
    def generate_beautiful_excel(self, excel_path):
        ...
        for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
            for cell in row:
                ...
                if cell.column == ws["rating"].column and cell.value >= 9.0:
        ...
        wb.save(excel_path)
        print(f"✅ 美化版Excel已生成：{excel_path}")
        return True
